...
class CSIROModel(nn.Module):
    ...
    def forward(self, x):
        ...
        total_g = pred_3[:, 2]  # (B,)
        dead = total_g - gdm  # (B,)
        clover = gdm - green  # GDM = Clover + Green より
        ...
        # 補助ターゲットもlog空間で予測
        aux_pred_raw = self.aux_head(x)  # (B, 1)
        aux_pred = torch.clamp(aux_pred_raw, min=0.0)  # 負値を防ぐ

        return pred_log, aux_pred
