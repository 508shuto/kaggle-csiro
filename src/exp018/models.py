import timm
import torch
import torch.nn as nn


class CSIROModel(nn.Module):
    def __init__(
        self,
        model_name: str = "efficientnet-b0",
        pretrained: bool = True,
        in_channels: int = 3,
        out_channels: int = 3,  # [clover_percent, dead_percent, green_percent, other_percent]
        aux_out_channels: int = 2,
    ):
        super().__init__()
        self.model = timm.create_model(
            model_name=model_name,
            pretrained=pretrained,
            in_chans=in_channels,
            global_pool="",
            num_classes=0,
        )
        self.pool = nn.AdaptiveAvgPool2d(1)

        self.percent_head = nn.Sequential(
            nn.Linear(self.model.num_features, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, out_channels),
        )
        self.amount_head = nn.Sequential(
            nn.Linear(self.model.num_features, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 1),
        )

        self.aux_head = nn.Sequential(
            nn.Linear(self.model.num_features, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, aux_out_channels),
        )

    def forward(self, x):
        x = self.model(x)
        x = self.pool(x).flatten(1)

        # 3つの予測: [Clover_percent, Dead_percent, Green_percent]
        components_percent = self.percent_head(x)  # (B, 3)

        # 量の予測: [Amount]
        amount = self.amount_head(x).squeeze(1)  # (B,)

        # 3つの予測を合計して1になるように正規化
        components_percent = torch.softmax(components_percent, dim=1)  # (B, 3)

        # 物理制約を厳密に守る
        # GDM = Clover + Green
        # Total = Clover + Dead + Green
        clover = components_percent[:, 0] * amount  # (B,)
        dead = components_percent[:, 1] * amount  # (B,)
        green = components_percent[:, 2] * amount  # (B,)
        total_g = amount  # (B,)
        gdm = total_g - dead  # (B,)

        # 5つに拡張: [Clover, Dead, Green, GDM, Total]
        pred = torch.stack([clover, dead, green, gdm, total_g], dim=1)  # (B, 5)
        pred = torch.clamp(pred, min=0.0)
        # log空間に戻す
        pred_log = torch.log1p(pred)  # (B, 5)

        # 補助ターゲットもlog空間で予測
        aux_pred_log = self.aux_head(x)  # (B, 2)

        return pred_log, aux_pred_log


if __name__ == "__main__":
    from argparse import ArgumentParser

    import torch

    parser = ArgumentParser()
    parser.add_argument("--model_name", type=str, default="resnet18")
    parser.add_argument("--pretrained", type=bool, default=False)
    parser.add_argument("--in_channels", type=int, default=3)
    parser.add_argument("--device", choices=["cpu", "mps", "cuda"], default="cpu")

    args = parser.parse_args()

    device = torch.device(args.device)
    model = CSIROModel(
        model_name=args.model_name,
        pretrained=args.pretrained,
        in_channels=args.in_channels,
        out_channels=3,
        aux_out_channels=2,
    ).to(device)
    model.eval()

    batch_size = 4
    channels = 3
    height = 256
    width = 256

    dummy_input = torch.randn(batch_size, channels, height, width).to(device)
    with torch.no_grad():
        pred, aux_pred = model(dummy_input)
    print(pred.shape)
    print(aux_pred.shape)
