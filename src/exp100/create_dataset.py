...
def create_metadata(df: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    ...
    for image_path in image_paths:
        ...
        # exp100では画像パスをそのまま使用（npyではなくjpg）
        data_items.append(
            DataItem(
                sample_id=sample_id,
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Creating StratifiedGroupKFold splits")
    print(f"  n_splits: {config.dataset.n_folds}")
    print(f"  shuffle: {config.dataset.shuffle}")
    print(f"  random_state: {config.experiment.seed}")
    print(f"  groups: sample_id")
    print(f"  stratify: species")
    print(f"{'='*60}\n")

    for fold, (_, val_idx) in enumerate(
        sgkf.split(X=df, y=df["species"], groups=df["sample_id"])
    ):
    ...
    for fold in range(config.dataset.n_folds):
        ...
        print(f"  Unique sample_ids: {fold_df['sample_id'].nunique()}")
        print(f"  Species distribution:")
        for species, count in fold_df["species"].value_counts().items():
            print(f"    {species}: {count} ({count/len(fold_df)*100:.1f}%)")
    ...
    print(f"Total unique sample_ids: {df['sample_id'].nunique()}")
    print(f"Species distribution:")
    for species, count in df["species"].value_counts().items():
        print(f"  {species}: {count} ({count/len(df)*100:.1f}%)")

    # Save data
    output_path = output_dir / "preprocessed_train.csv"
