...
class CSIROModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mixup_enabled and torch.rand(1).item() < self.mixup_prob:
            image, targets, lam = mixup_batch(image, targets, alpha=self.mixup_alpha)
            self.log("train_mixup_lambda", lam, on_step=False, on_epoch=True)

        # 対数空間で予測・損失計算
        pred_log = self.model(image)
        loss = self.loss_fn(pred_log, targets)

        self.log("train_loss", loss, prog_bar=True, on_step=False, on_epoch=True)
        return loss

    def validation_step(self, batch, batch_idx):
        image, targets_log = batch

        # 対数空間で予測
        preds_log = self.model_ema.module(image)  # (B, 5)

        # 対数空間でLoss計算
        loss = self.loss_fn(preds_log, targets_log)
        # 元の空間に戻してメトリクス計算
