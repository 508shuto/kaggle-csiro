    ...
    for batch in tqdm(dataloader, desc=f"Fold {fold}"):
        ...
        if use_tta and use_amp and device_type == "cuda":
            with torch.no_grad():
                with torch.amp.autocast(device_type=device_type):
                    # Horizontal flip
                    hflip_pred_log = model(torch.flip(images, dims=[3]))
                    batch_predictions_log.append(hflip_pred_log.cpu().detach())
                    # Vertical flip
                    vflip_pred_log = model(torch.flip(images, dims=[2]))
                    batch_predictions_log.append(vflip_pred_log.cpu().detach())
                    # Rot90
                    rot90_pred_log = model(torch.rot90(images, k=1, dims=[2, 3]))
                    batch_predictions_log.append(rot90_pred_log.cpu().detach())
                    # Rot270
                    rot270_pred_log = model(torch.rot90(images, k=3, dims=[2, 3]))
