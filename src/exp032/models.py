...
class RegressionHeadOnly(nn.Module):
    """Regression head-only model for cached feature training.

    This lightweight model contains only the regression heads without the vision encoder.
    Used for fast training when vision features are pre-extracted and cached.
    """
    ...
    def forward(self, pooled_features: torch.Tensor):
        """Forward pass with pre-extracted pooled features.

        Args:
            pooled_features: Pooled vision features of shape (B, hidden_dim)
        ...
        """
        # Predict 3 base targets: [Clover, Dead, Green]
        pred_3 = self.head(pooled_features)  # (B, 3)
        ...
        # Auxiliary predictions
        aux_pred = self.aux_head(pooled_features)  # (B, 2)
