...
from models import Qwen3VLRegressionModel, RegressionHeadOnly
...
class CachedCSIROModule(L.LightningModule):
    """Lightning module for cached feature-based training.

    Uses pre-extracted vision features for fast head-only training.
    """

    def __init__(self, config: DictConfig):
        super().__init__()
        self.config = config
        self.model = RegressionHeadOnly(
            hidden_dim=config.model.hidden_dim,
            head_hidden_dim=config.model.head_hidden_dim,
            dropout=config.model.dropout,
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        features, targets, aux_targets = batch

        # Forward pass
        preds, aux_preds = self.model(features)

        # Compute loss (cache aux_loss for logging)
        aux_loss = self.aux_loss_fn(aux_preds, aux_targets)
        loss = self.loss_fn(preds, targets) + self.aux_weight * aux_loss

        self.log("train_loss", loss, prog_bar=True, on_step=False, on_epoch=True)
        self.log("train_aux_loss", aux_loss, prog_bar=True, on_step=False, on_epoch=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        preds, aux_preds = self.model_ema.module(features)
        ...
        self.log("val_loss", loss, prog_bar=True, on_step=False, on_epoch=True)
        self.log("val_aux_loss", aux_loss, prog_bar=True, on_step=False, on_epoch=True)
        return loss
    ...
    def configure_optimizers(self):
        # All parameters are trainable in head-only model
