...
class CachedFeatureDataset(Dataset):
    """Dataset for cached feature-based training.

    Loads pre-extracted vision encoder features instead of images.
    This enables fast head-only training by skipping image loading and vision encoder forward.
    """

    def __init__(self, config: DictConfig, df: pd.DataFrame, feature_dir: str | Path, mode: str):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feature_path = self.feature_dir / f"{image_id}.npy"
        ...
        return feature, target, aux_target
