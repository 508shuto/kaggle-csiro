...
def predict(
    ...
    for batch in tqdm(dataloader, desc=f"Fold {fold}"):
        ...
        # 各予測を負値除去してから平均（TTA使用時）
        batch_predictions_processed = []
        for pred_tensor in batch_predictions:
            pred_processed = torch.clamp(pred_tensor, min=0.0)  # 負値除去
...
def main(
    ...
    config_path: Path = Path("./config/exp043.yaml"),
    model_dir: Path = Path("./output/exp043"),
    output_dir: Path = Path("./output/exp043"),
