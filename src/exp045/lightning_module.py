...
class CSIROModule(L.LightningModule):
    def __init__(
        ...
        self.unfreeze_blocks = config.model.get("unfreeze_blocks", None)
    ...
    def configure_optimizers(self):
        ...
        # Partial Unfreeze時はparam_groupsを分離
        if isinstance(trainable_params, dict):
            # backbone と head で異なる学習率を適用
            backbone_lr = self.config.trainer.train.get("backbone_lr", 1e-5)
            head_lr = self.config.trainer.train.get("head_lr", 1e-3)
            weight_decay = self.config.trainer.train.optimizer.get("weight_decay", 1e-2)

            param_groups = [
                {
                    "params": trainable_params["backbone"],
                    "lr": backbone_lr,
            ...
            optimizer = torch.optim.AdamW(
                param_groups,
                betas=tuple(self.config.trainer.train.optimizer.get("betas", [0.9, 0.999])),
                eps=self.config.trainer.train.optimizer.get("eps", 1e-8),
                amsgrad=self.config.trainer.train.optimizer.get("amsgrad", False),
            )
            print(f"Partial Unfreeze optimizer: backbone_lr={backbone_lr}, head_lr={head_lr}")
        else:
            # 従来通りの処理（Freeze時 or 全学習時）
