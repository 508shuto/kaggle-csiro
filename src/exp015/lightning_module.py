...
class CSIROModule(L.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.config = config
        # MIL設定の取得（存在しない場合はNone）
        mil_config = config.model.get("mil", {})
        patch_size = mil_config.get("patch_size")
        stride = mil_config.get("stride")
        attn_hidden_dim = mil_config.get("attn_hidden_dim", 256)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss_fn(pred_log, targets) + self.aux_weight * self.aux_loss_fn(aux_pred_log, aux_targets)
