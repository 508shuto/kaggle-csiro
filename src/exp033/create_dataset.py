...
def main():
    ...
    # Split data
    df["fold"] = -1
    # Use StratifiedKFold with State + target_bin stratification
    df["target_bin"] = pd.qcut(df["total_target"], q=3, labels=["low", "mid", "high"])
    df["stratify_key"] = df["state"] + "_" + df["target_bin"].astype(str)
    skf = StratifiedKFold(
        n_splits=config.dataset.n_folds,
        shuffle=True,
        random_state=config.experiment.seed,
    )
    for fold, (_, val_idx) in enumerate(skf.split(df, y=df["stratify_key"])):
        df.loc[val_idx, "fold"] = fold
    # Remove temporary columns
    df = df.drop(columns=["target_bin", "stratify_key"])
