...
def build_test_index(test_csv_path: Path, input_dir: Path) -> pd.DataFrame:
    ...
    Args:
        ...
        input_dir: Input directory containing original image files
    ...
    for image_path in image_paths:
        ...
        full_image_path = input_dir / image_path
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Load image directly from JPEG instead of npy
        image = np.array(Image.open(row["image_path"]).convert("RGB"), dtype=np.float32)
...
def main(
    ...
    config_path: Path = Path("./config/exp019.yaml"),
    model_dir: Path = Path("./output/exp019"),
    output_dir: Path = Path("./output/exp019"),
