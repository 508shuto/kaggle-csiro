...
def main():
    ...
    # exp042: Main Species方式のCV戦略
    # Main Species = species名の"_"分割1つ目 (例: Ryegrass_Clover → Ryegrass)
    df["main_species"] = df["species"].apply(lambda x: x.split("_")[0])

    # グループ: sampling_date + state + main_species
    # → 同一日+同一State+同一Main Speciesは必ず同じFoldに入る
    groups = df["sampling_date"] + "_" + df["state"] + "_" + df["main_species"]

    # 層化キー: state + main_species
    df["state_main_species"] = df["state"] + "_" + df["main_species"]
    ...
    # 一時列を削除
    df = df.drop(columns=["main_species", "state_main_species"])
