...
```bash
# 1. 画像前処理
python src/exp023/img2npy.py \
  --input-dir ./input/train \
  --output-dir ./output/exp023/train \
  --image-size 512 \
  --num-workers 4

python src/exp023/img2npy.py \
  --input-dir ./input/test \
  --output-dir ./output/exp023/test \
  --image-size 512 \
  --num-workers 4

# 2. データセット作成
python src/exp023/create_dataset.py

# 3. 訓練（5 folds）
python src/exp023/train.py --config-path ./config/exp023.yaml

# 4. OOF評価
python src/exp023/evaluation.py

# 5. テストセット推論
python src/exp023/inference.py \
  --config-path ./config/exp023.yaml \
  --test-csv-path ./input/test.csv \
  --output-dir ./output/exp023 \
  --device cuda \
  --batch-size 256 \
  --num-workers 4 \
  --use-amp true
...
for fold in range(5):
    # EMAモデルをロード
    model = load_checkpoint(f'output/exp023/fold{fold}.ckpt')
...
```python
# EXP023 + EXP005
final = 0.6 * exp023_pred + 0.4 * exp005_pred
```
